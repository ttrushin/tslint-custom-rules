...
export class Rule extends Lint.Rules.AbstractRule {
  ...
  public static metadata: Lint.IRuleMetadata = {
    ruleName: "react-dont-mutate-props",
    description: "Forbidden the mutation of props in a component",
  ...
  public static FAILURE_STRING = "Do not mutate props.";
...
function walk(ctx: Lint.WalkContext<void>) {
  return ts.forEachChild(ctx.sourceFile, function cb(node: ts.Node): void {
    if (
      ...
      node.left.getText().indexOf(".props.") > -1 // verify that we are accessing `this.props.SOMETHING`, specifically
    ) {
